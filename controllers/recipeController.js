...
export const getRecipes = async (req, res, next) => {
    try {
        ...
        const recipes = await Recipe.findAll(req.user.id, { //findByUserId is now findAll with filters
            search,
            cuisine_type,
            difficulty,
            dietary_tag,
...
export const getRecentRecipes = async (req, res, next) => {
    try {
        ...
        const recipes = await Recipe.findRecent(req.user.id, limit);//getRecent
        res.json({
            success: true,
            data: { recipes }
        });
